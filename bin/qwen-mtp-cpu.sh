...
#
# Node 1 runs two of these, one slot each. A slot reading a long prompt blocks
# every other slot on its instance. A second slot only queues behind the
...
ARGS=(
  ...
  "${MTP_ARGS[@]}"
  "${VISION_ARGS[@]}"
  # Quoted: these values come from the caller's environment through
  # bin/restart-backend.sh. An unquoted empty one leaves its flag without a
  # value.
  --parallel "${SLOTS:-1}"
  "${KV_ARGS[@]}"
  --batch-size "${BATCH:-512}"
  --ubatch-size "${UBATCH:-512}"
                       # Each expert sees UBATCH rows at a time. Two instances
                       # on one socket, aggregate tokens/s: 43.6 at 512 against
                       # 41.0 at 2048 reading 8192 tokens; 35.0 against 34.3
                       # at 32768. Never worse at any length measured.
